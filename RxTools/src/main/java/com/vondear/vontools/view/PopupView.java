...
import com.vondear.vontools.bean.ActionItem;
...
public class PopupView extends PopupWindow {
    ...
    public PopupView(Context context, int width, int height) {
        ...
        // 获得屏幕的宽度和高度
        mScreenWidth = VonDeviceUtils.getScreenWidth(mContext);
        mScreenHeight = VonDeviceUtils.getScreenHeight(mContext);
    ...
    public PopupView(Context context, int width, int height, int layout) {
        this.mContext = context;
    ...
    public void show(View view) {
        ...
        // 显示弹窗的位置
        showAtLocation(view, popupGravity, mScreenWidth - LIST_PADDING - (getWidth() / 2), mRect.bottom + VonImageUtils.dp2px(mContext, 7.5f));
    }
